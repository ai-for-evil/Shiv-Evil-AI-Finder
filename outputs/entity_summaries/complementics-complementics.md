# Complementics. Complementics

- Code: Not included
- Subgroup: Not included
- Confidence: 0.35
- Review status: low_confidence

## Rationale

Evidence was too weak or ambiguous to assign a taxonomy code confidently.

## Evidence

- Complementics. Complementics is a data broker that captures mobile location data, extracting detailed insights from apps installed on users’ smartphones. According to a report by VICE, Complementics collected location data from mobile applications and funneled it to firms like Venntel, which in turn sold this data to U.S. government agencies including ICE, DHS, and the FBI. This allowed federal agencies to circumvent warrant requirements and tr

## Sources

- https://misp-galaxy.org/surveillance-vendor/